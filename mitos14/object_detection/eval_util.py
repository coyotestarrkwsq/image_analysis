...
def run_checkpoint_once(training_process,
                        mitosis_map, 
                        mitosis_class,
                        tensor_dict,
                        update_op,
  ...
  with tf.contrib.slim.queues.QueueRunners(sess):
    try:
      ...
      mitosis = ('PerformanceByCategory/mAP@{}IOU/{}'
                      .format(0.5, mitosis_class))
      mitosis_metric = metrics[mitosis]
      if get_map(mitosis_map) < mitosis_metric:
        update_map(mitosis_map, mitosis_metric)
      elif get_map(mitosis_map) >= mitosis_metric:
        increment_freq(mitosis_map)
      
      print mitosis_map
      
      if get_map_freq(mitosis_map) >= 19:
        subprocess.call('kill ' + training_process, shell = True)
        sys.exit()
...
# TODO: Add tests.
def repeated_checkpoint_run(training_process,
                            mitosis_map, 
                            mitosis_class,
                            tensor_dict,
                            update_op,
  ...
  while True:
    ...
    if not model_path:
      ...
      run_checkpoint_once(training_process,  mitosis_map, mitosis_class,
                          tensor_dict, update_op, summary_dir,
...
def get_map(mitosis_map):
  return mitosis_map[0]

def get_map_freq(mitosis_map):
  return mitosis_map[1]

def update_map(mitosis_map, new_map):
  mitosis_map[0] = new_map
  mitosis_map[1] = 0
    
def increment_freq(mitosis_map):
  mitosis_map[1]+=1